<script>
  ...
  async function handleDelete() {
    try {
      const response = await apiFetch(`/api/category/abbreviation/` + encodeURIComponent(value), {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      console.error(error);
      toast.push("Erreur lors de la suppression de l'abbreviation", {
        theme: {
          "--toastBackground": "#f44336",
          "--toastColor": "#fff",
        },
    ...
    toast.push("Abbreviation supprimée avec succès", {
      theme: {
        "--toastBackground": "#4caf50",
        "--toastColor": "#fff",
      },
