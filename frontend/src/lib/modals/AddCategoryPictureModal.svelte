<script>
  ...
  import { categories } from "$lib/stores/searches";
  import Icon from "@iconify/svelte";

  const { isOpen, close, category, index } = $props();
  ...
  async function submitForm() {
    ...
    formData.append("referenceId", category.id);
          ...
          categories.update((cats) => {
            cats.forEach((cat) => {
              if (cat.id === category.id) {
                console.log("cat.id: ", cat.pictureId);
