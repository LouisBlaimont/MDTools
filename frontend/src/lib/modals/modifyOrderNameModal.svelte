<script>
    ...
    const {
      ...
      orderName // The order name passed as a prop
    } = $props();
  
    let newOrderName = $state(orderName || "");
    ...
    async function modifyOrder(newOrderName) {
      ...
      try {
        ...
        if (response.status === 400) {
          ...
          throw new Error(`Failed to modify order: ${response.status}`);
        ...
        toast.push($_("modals.edit_order.success"));
        close();
      } catch (error) {
        ...
        >
          ...
          >
            <h2 class="text-2xl font-bold text-teal-500 text-center">
              {$_("modals.edit_order.edit_title")} "{orderName}"
            </h2>
            <!-- Edit Icon -->
            <svg
          ...
          {#if loading}
            <div role="status" class="my-6 flex items-center justify-center p-4">
              ...
              <span class="sr-only">{$_('modals.edit_order.loading')}</span>
            </div>
          {:else}
            <form on:submit|preventDefault={handleModifyOrder} class="bg-gray-100 p-6 rounded-b-lg">
              <div class="mt-4">
                <label for="new-order-name" class="block text-lg font-semibold text-gray-700">
                  {$_("modals.edit_order.name")}:
                </label>
                <input
                  type="text"
                  id="new-order-name"
                  bind:value={newOrderName}
                  class="w-full p-2 border rounded mb-4 mt-2"
                  placeholder={$_("modals.edit_order.name_placeholder")}
                />
                <span id="error-no-name" class="mb-5 text-red-600 text-sm hidden">
                  {$_("modals.edit_order.error_no_name")}
                </span>
                <span id="error-no-order-for-user" class="mb-5 text-red-600 text-sm hidden">
                  {$_("modals.edit_order.error_already_exists")}
                </span>
              </div>
