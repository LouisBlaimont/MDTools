...
>
  <div class="absolute inset-0 bg-gray-500/75 transition-opacity" aria-hidden="true"></div>
    ...
    <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:w-full sm:max-w-lg lg:max-w-4xl p-6">
        ...
        <h2 class="text-lg font-semibold text-gray-900 text-center" id="modal-title">{$_("modals.create_order.create_title")} :</h2>

        <div class="mt-4">
            <label for="new-order-name" class="block text-sm font-medium text-gray-700">{$_("modals.create_order.name")} :</label>
            <input
                type="text"
                id="new-order-name"
                class="mt-1 mb-2 block w-full border border-gray-300 rounded-md p-2 text-gray-900"
            />
            <span id="error-no-name" class="mb-5 text-red-600 text-sm hidden">{$_("modals.create_order.error_no_name")}</span>
            <span id="error-no-order-for-user" class="mb-5 text-red-600 text-sm hidden">{$_("modals.create_order.error_already_exists")}</span>
        </div>
