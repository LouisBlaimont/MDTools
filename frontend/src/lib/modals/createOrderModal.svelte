        ...
        .then((result) => {
            ...
            for(const order of result){
                if(order.name === orderName){
                    ...
                    orders.update(currentOrders => [...currentOrders, orderDTO])
                    break;
                }
            }
            close();
...
>
  <div class="absolute inset-0 bg-gray-500/75 transition-opacity" aria-hidden="true"></div>
    ...
    <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:w-full sm:max-w-lg lg:max-w-4xl p-6">
        ...
        <h2 class="text-lg font-semibold text-gray-900 text-center" id="modal-title">Créer une nouvelle commande :</h2>

        <div class="mt-4">
            <label for="new-order-name" class="block text-sm font-medium text-gray-700">Nom de la nouvelle commande :</label>
            <input
                type="text"
                id="new-order-name"
